...
function tmip_static_urls() {
	if (defined('tmip_home_page_url')) return NULL;
	...
	define("tmip_home_page_login", 		
		   $tmip_prot.'://www.'.tmip_domain_name.'?rnDs=70124045620&wMx=1&rFsrc='.$tmip_rFsrc);
	...
	define("tmip_free_signup_page_url", 
		   tmip_service_url.'/tools/website-visitors-counter-traffic-tracker-statistics/index.php?sto=1&wpp=plug_settings_page');
	define("tmip_premium_signup_page_url", 
			tmip_service_url.'/members/index.php?page=spm_checkout&type=ssub&ntc=1&wpp=plug_settings_page');
	...
	define("tmip_kb_rock_ldr",
		   '<a href="'.tmip_wp_learn_kb.'disable-rocket-loader-to-enable-visitor-ip-tracking-936/?wpp=plug_settings_page" target="_blank"><b>Rocket Loader</b></a>');
	
	// Learn KB urls 071323070147
	define("tmip_wp_vistr_setup_tutorial", 
			tmip_wp_learn_kb.'wordpress-visitor-tracking-plugin-1382/?wpp=plug_settings_page');
	define("tmip_wp_vistr_troubleshooting_tutorial", 
			tmip_wp_learn_kb.'wordpress-plugin-stopped-working-2671/?wpp=plug_settings_page');	define("tmip_wp_block_ips_visit_control",
			tmip_wp_learn_kb.'how-to-block-an-ip-address-1017/?wpp=plug_settings_page');
	define("tmip_wp_other_useful_tutorials",
			tmip_wp_learn_kb.'most-popular-tracemyip-features-1594/?wpp=plug_settings_page');
	define("tmip_wp_website_cache_optimization",
			tmip_wp_learn_kb.'website-caching-cdn-and-website-analytics-and-visitor-tracking-issues-2422/?wpp=plug_settings_page');
	define("tmip_chart_tracker_url",     
		   $tmip_prot.'://tools.'.tmip_domain_name.'/ipbox/md=1&pID=1-324473478-1-1600~1587599353~300*100*1*1*20~8CFFD1*33313B~0*0*0&uID=3-042220&pgB=013201&ttC=FF0000&trT=FFFFFF&erA=AED99E&orA=DEEFD7&nrC=666666&vrC=006666&sOP=icstzdlrpob&ipbCap=1&plVer='.$wp_pl_version.'&wpVer='.$wp_wp_version.'&wpAem='.$wp_user_info_ae);
	...
	define("tmip_db_options_vars", 	
		array(
			...
			'vis_tracker_ploads_ou_opt'=>	array('db_var'=>'tmip_stat_vis_tr_query',			'default'=>		0),
			'vis_tracker_ploads_lifetime'=>	array('db_var'=>'tmip_stat_vis_tr_total_qry',		'default'=>		0,	'no_reset'=>1),
	...
	define("tmip_page_tracker_opt", 		tmip_db_options_vars['page_tracker_code']['db_var']);
...
function tmip_access_reports(){
	...
	if (1==1) {
		add_submenu_page(
			$menuID,
			tmip_service_Nname.' > '.ucwords(tmip_submenu_user_guide),
			ucwords(tmip_submenu_user_guide),	
			'manage_options',
			'tmip_lnk_user_guide_lrn',
			'tmip_fnc_user_guide'
		);
	}
	if (1==2) { // Not used.
...
function tmip_fnc_user_guide() {
	tmip_load_js();
	tmip_wp_iframe_page(tmip_userguide_url);
}
...
function tmip_insert_page_tracker() {
    $code=get_option(tmip_page_tracker_opt);
    if ($code and strpos($code,'tP_lanPTl') !== false) {
		$code=stripslashes($code);
		$code=str_replace('src="http:','src="',$code);
		$code=str_replace('src="https:','src="',$code);
		if (!tmip_is_front_end_page()) tmip_log_stat_data(array('type'=>'pag_tr_query'));
		echo $code;
    }
}
function tmip_log_stat_data($input) {
	...
	if ($type=='vis_tr_query' or $type=='vis_tr_reset' or $type=='pag_tr_query' or $type=='pag_tr_reset' or $type=='reset_all_data') {
		if ($type=='vis_tr_query' or $type=='vis_tr_reset' or $type=='reset_all_data') {
			$database_column1=tmip_vis_trk_ploads_curr_opt;
			$database_column2=tmip_vis_trk_code_html_js_opt;
			$database_column3=tmip_pag_trk_ploads_curr_opt;
			$database_column4=tmip_vis_trk_first_use_opt;
		} elseif ($type=='pag_tr_query' or $type=='pag_tr_reset') {
			$database_column1=tmip_page_ploads_ou_opt;
		}
		...
		if ($action=='write') {
			...
			// This was previously sanitized. strip tags is not intended to sanitize and is used for a different purpose.
			if ($database_column2) update_option($database_column2, strip_tags(esc_js($js_conversion))); 
			...
			tmip_reset_delete_plugin_data(2);
		}
	...
	// Visitor tracker
		...
		$output['vis_tr_queries_cnt']=(int)get_option(tmip_vis_trk_ploads_curr_opt);
		if ($v=get_option(tmip_vis_trk_code_html_js_opt)) {
			$v=stripslashes($v);
		} else {
			$v=NULL;
		}
		$output['vis_tr_htmljs_conv']=$v;
		$output['vis_tr_first_use_unix']=(int)get_option(tmip_vis_trk_first_use_opt);
		$output['vis_tr_total_queries']=(int)get_option(tmip_pag_trk_ploads_curr_opt);
		
		if ($v=$output['vis_tr_queries_cnt']) {
			$activity=tmip_stats_receive_trdata;
			if (tmip_codes_usage_stats_data==2) $activity .=' <span class="tmip_trk_usage_stat_num">( '.($v/1).' )</span>';
		} else {
		...
		if ($v=$output['vis_tr_htmljs_conv']) {
			$output['vis_tr_stats'] .='<br><b>'.tmip_stats_status_pagntra.'</b>: '.$v;
		}
		return $output;
	
	// Page tracker
	} elseif (tmip_codes_usage_stats_data and $type=='pag_tr_stats') {
		$output['pag_tr_queries_cnt']=(int)get_option(tmip_page_ploads_ou_opt);
...
function tmip_settings_page() {
	...
	if (!$vtpTRpsDbVar or ($info_update and !trim(sanitize_text_field($_POST[tmip_visit_tracker_opt])))) {
	...
	$vistracker_db_source_code=trim(stripslashes(get_option(tmip_visit_tracker_opt)));
	$pagtracker_db_source_code=trim((get_option(tmip_page_tracker_opt)));
	...
	*/
	...
	$postVarVisTr=NULL;
	if (isset($_POST[tmip_visit_tracker_opt])) {
		$postVarVisTr=$_POST[tmip_visit_tracker_opt];
		/*
			Bug report 101721053351  // wp_kses() bug converts &amp; to &#038; on wordpress hosted sites and ommits javascript tag
			$postVarVisTr=wp_kses($_POST[tmip_visit_tracker_opt],$allowed_html,$allowed_protocols);
		*/		
	...
	if ($vtpTRpsDbVar or $postVarVisTr or ($info_update and !$postVarVisTr)) {
		...
		//  or $source_code=get_option(tmip_visit_tracker_opt)
	...
	$postVarPagTr='';
	if (isset($_POST[tmip_page_tracker_opt])) {
		$postVarPagTr=trim($_POST[tmip_page_tracker_opt]);
		if (strpos($postVarPagTr,'4684NR-IPIB')==false) {
			$postVarPagTr=tmip_sanitize_rebuild_page_tracker_code($_POST[tmip_page_tracker_opt]);
			$postVarPagTr=str_replace('/script>"','\\\/script>"',$postVarPagTr);
		}
	}
	//$postVarPagTr=esc_js(tmip_remove_tabs_new_lines($_POST[tmip_page_tracker_opt]));
	//$postVarPagTr=tmip_unify_new_lines(trim(stripslashes($postVarPagTr)));
	//$postVarPagTr=html_entity_decode($postVarPagTr);
	//$postVarPagTr=str_replace('/script>"','\\\/script>"',$postVarPagTr);

	$pagTRpsDbVar=!empty($postVarPagTr) ? $postVarPagTr : get_option(tmip_page_tracker_opt);
	...
	if ($failedNonceCheck<>1) {
		...
		if ($info_update and (!$postVarVisTr and get_option(tmip_visit_tracker_opt)) or 				
													stristr($haltUpdate,'hu_visitor_tracker'))	{
			if ($failedNonceCheck<>1 and $allowUpdate) {
				update_option(tmip_visit_tracker_opt,'');
		...
		if ($info_update and (!$postVarPagTr and get_option(tmip_page_tracker_opt)) or stristr($haltUpdate,'hu_page_tracker')) {
			if ($allowUpdate) {
				update_option(tmip_page_tracker_opt,'');
		...
		if ($proceedToUpdate and isset($info_update) and 
			...
			if ($allowUpdate) {
				update_option(tmip_visit_tracker_opt,$tmip_visit_tracker);
				tmip_log_stat_data(array('type'=>'vis_tr_reset'));
			}
			...
			if ($allowUpdate) {
				update_option(tmip_page_tracker_opt,$tmip_page_tracker);
				tmip_log_stat_data(array('type'=>'pag_tr_reset'));
			}
			...
			<blockquote>
				...
				<section>
					<ul class="tmip_sett_list_ol">
						...
						<li><span>Website visitor visual and sound alerts, redirecting, and blocking based on custom rules by an IP address, computer ID, connection location, number of visits or page views, browser type, operating system, referrers, computer hardware specifications, etc.</span></li>
						<li><span>Mobile and desktop device statistics, targeting, redirection and browsing path control</span></li>
						<li><span>Individual Real-Time one-way message and popup alerts delivery for specific website visitors</span></li>
						<li><span>Web page, links, documents and contact forms protection. Advanced IP based tracking giving you a complete access control over your online content security</span></li>
						<li><span>Integrated and secure IP tracking data visitor control interface within WordPress dashboard</span></li>
						<li><span>EU GDPR Data Processing compliance tools with variable level of restrictions per jurisdiction</span></li>
			...
			<blockquote>
				...
				<section>
					<ol class="tmip_sett_list_ul">
						...
						<br><span class="tmip_note_small"><i>* If you need to change a tracker style for an existing project, login to your account, go to [My Projects] page and click on the "tracker code" link located to the right of the project\'s name.</i></span></li>
						...
						<li><span><b>Access</b> your '.tmip_service_Dname.' console, login to WordPress and go to [Dashboard] => [TraceMyIP > '.tmip_submenu_reports.']</span></li>
					</ol>
				...
				<span class="tmip_note_small"><b>Optional:</b> To block IPs and control visitor access, install a Page Tracker code (refer to the tutorial links provided below).</span>
					<p><b>Step-by-step tutorials:</b>
						<ul class="tmip_sett_list_ul">
							<li>Installing <a href="'.tmip_wp_vistr_setup_tutorial.'" target="_blank"><b>WordPress Visitor Tracker</b></a> and <a href="'.tmip_wp_vistr_troubleshooting_tutorial.'" target="_blank"><b>troubleshooting</b></a></li>
					...
					<table id="tmip_sett_area">
						...
						<tr>
							<td valign="top" style="padding-top:8px;" width="200">
								<label for="'.tmip_visit_tracker_opt.'" class="tmip_input_box_name">'.tmip_lang_visitor_tr_code.':</label>
							...
							<td>
								<textarea id="'.tmip_visit_tracker_opt.'" name="'.tmip_visit_tracker_opt.'" onClick="tmip_select_all(\''.tmip_visit_tracker_opt.'\');" class="tmip_textarea" placeholder="'.trim(tmip_vis_trk_inp_placehl).'"'.$dcach.'>'.htmlentities(stripslashes(get_option(tmip_visit_tracker_opt))).'</textarea>
						...
						<tr>
							...
							<td>
								<textarea id="'.tmip_page_tracker_opt.'" name="'.tmip_page_tracker_opt.'" onClick="tmip_select_all(\''.tmip_page_tracker_opt.'\');" class="tmip_textarea" placeholder="'.trim(tmip_pag_trk_inp_placehl).'"'.$dcach.'>'.stripslashes(get_option(tmip_page_tracker_opt)).'</textarea>
...
function tmip_reset_delete_plugin_data($data_type=1) {
    global $wpdb;
	
	# data_type: 1-reset all except no_reset flagged vars, 2-reset all to defaults, 3-delete all options, 4-add DB options defaults
	$operation=array();
	$deleted_options=array();
	
	// Reset settings to default
	if ($data_type===1 or $data_type===2) {
		foreach (tmip_db_options_vars as $vars) {
			$no_reset=(isset($vars[$a='no_reset']) and $data_type===1 and $v=$vars[$a] and $v===1) ? $v : 0;
			if (isset($vars['db_var']) and isset($vars['default']) and $no_reset<>1) {
				update_option($vars['db_var'],trim($vars['default']));
				$reset_options[$vars['db_var']]=$vars['default'];
			}
		}
	...
	// Delete all options from database
		...
		foreach (tmip_db_options_vars as $vars) {
			if (isset($vars['db_var']) and tmip_does_option_exist($vars['db_var'])) {
				delete_option($vars['db_var']);
				$deleted_options[]=$vars['db_var'];
			}
		}
		$deprecated_options=array('tmip_position','tmip_page_tracker_opt','tmip_visit_tracker_opt');
		foreach ($deprecated_options as $db_opt) {
			if (isset($db_opt)) {
				delete_option($db_opt);
				$deleted_options[]=$db_opt;
			}
		}
	...
	// Add options to database
		...
		foreach (tmip_db_options_vars as $vars) {
			if (isset($vars['db_var']) and !tmip_does_option_exist($vars['db_var'])) {
				add_option($vars['db_var'],trim($vars['default']));
				$add_options[$vars['db_var']]=$vars['default'];			
			}
		}
	}
	
	if (!empty($deleted_options)) 	$operation['deleted_options']=$deleted_options;
	if (!empty($reset_options)) 	$operation['reset_options']=$reset_options;
	if (!empty($add_options)) 		$operation['added_options']=$add_options;
	
	return $operation;
}

function tmip_does_option_exist($option_name) {
    global $wpdb;
...
function tmip_is_front_end_page() {
    ...
    if ($req) {
        ...
        if (is_admin() or preg_match($regex, $req)) {
            return 1;
        }
    }
    return 0;
...
function tmip_write_debug_info_to_file($vars) {
	if (1) {
        if (!is_array($vars)) $vars = array($vars);
        $plugin_dir = plugin_dir_path(__FILE__);
        $file_path = $plugin_dir . 'tmip_debug_info.txt';
        $counter_file_path = $plugin_dir . 'tmip_counter.txt';
        $current_time = date('Y-m-d H:i:s') . '.' . sprintf('%03d', (int) (microtime(true) * 1000) % 1000);
        $instance_number = 0;
        $fp = fopen($counter_file_path, 'c+');
        if ($fp) {
            if (flock($fp, LOCK_EX)) {
                if (filesize($counter_file_path) > 0) {
                    $counter_value = fgets($fp);
                    $instance_number = $counter_value ? (int)$counter_value : 0;
                } else {
                    $instance_number = 0;
                }
                $instance_number++;
        ...
        foreach ($vars as $key => $value) {
			if (!empty($key) and is_string($key) and is_string($value))  $debug_info .= 'key ['.$key . ']: ' . $value . "\n";
        }
